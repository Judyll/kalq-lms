...
{
    partial class BankInformationCreate
    {
        ...
        public BankInformationCreate(CommonExchange.SysAccess userInfo, DisbursementVoucherLogic disbursementManager)
        ...
        //####################################################CLASS BankInformationCreate EVENTS###############################################
        //event is raised when the class is clossing
        private void ClassClossing(object sender, FormClosingEventArgs e)
        {
            if (!_hasCreated)
            {
                String strMsg = "Are you sure you want to cancel the creation of the new bank information?";
        ...
        //####################################################END CLASS BankInformationCreate EVENTS###############################################
        ...
        private void btnCreateClick(object sender, EventArgs e)
        {
            if (this.ValidateControls())
            {
                try
                {
                    String strMsg = "Are you sure you want to create a bank information?";

                    DialogResult msgResult = MessageBox.Show(strMsg, "Confirm Create", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                    if (msgResult == DialogResult.Yes)
                    {
                        strMsg = "The bank information has been successfully created.";

                        this.Cursor = Cursors.WaitCursor;

                        _bankInfo.ObjectState = DataRowState.Added;

                        _disbursementManager.InsertBankInformation(_userInfo, _bankInfo);
