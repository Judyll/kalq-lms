...
{
    partial class CollateralInformationCreate
    {
        ...
        public CollateralInformationCreate(CommonExchange.SysAccess userInfo, CommonExchange.Member memberInfo, MemberLogic memberManager)
        ...
        private void ClassClossing(object sender, FormClosingEventArgs e)
        {
            if (!_hasCreated)
            {
                String strMsg = "Are you sure you want to cancel the creation of the new collateral information?";
        ...
        private void btnCreateClick(object sender, EventArgs e)
        {
            if (this.ValidateControls())
            {
                try
                {
                    String strMsg = "Are you sure you want to create a collateral information?";

                    DialogResult msgResult = MessageBox.Show(strMsg, "Confirm Create", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                    if (msgResult == DialogResult.Yes)
                    {
                        strMsg = "The collateral information has been successfully created.";

                        this.Cursor = Cursors.WaitCursor;

                        _collateralInfo.ObjectState = DataRowState.Added;

                        _memberManager.InsertCollateralInformation(_userInfo, _collateralInfo);
