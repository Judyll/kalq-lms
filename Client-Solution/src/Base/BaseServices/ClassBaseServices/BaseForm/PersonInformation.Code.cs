...
namespace BaseServices
{
    partial class PersonInformation : IDisposable
    {
        ...
        protected CommonExchange.SysAccess _newUserInfo;
        ...
        private Int32 _primaryIndexSpouce = 0;
        private Int32 _rowIndexSpouce = -1;
        private String _primaryIdSpouce = String.Empty;
        #endregion
        ...
        public PersonInformation(CommonExchange.SysAccess userInfo, BaseServicesLogic baseManager)
        {
            this.InitializeComponent();

            _userInfo = userInfo;
            _baseServiceManager = baseManager;
            ...
            this.pbxGeneralInformation.MouseLeave += new EventHandler(pbxGeneralInformationMouseLeave);
            this.pbxGeneralInformation.MouseEnter += new EventHandler(pbxGeneralInformationMouseEnter);
            this.pbxPersonImage.Click += new EventHandler(pbxPersonImageClick);
            this.lnkCreateSpouce.LinkClicked += new LinkLabelLinkClickedEventHandler(lnkCreateSpouceLinkClicked);
            this.dgvPersonSpouce.MouseDown += new MouseEventHandler(dgvPersonSpouceMouseDown);
            this.dgvPersonSpouce.KeyPress += new KeyPressEventHandler(dgvPersonSpouceKeyPress);
            this.dgvPersonSpouce.KeyDown += new KeyEventHandler(dgvPersonSpouceKeyDown);
            this.dgvPersonSpouce.DataSourceChanged += new EventHandler(dgvPersonSpouceDataSourceChanged);
            this.dgvPersonSpouce.SelectionChanged += new EventHandler(dgvPersonSpouceSelectionChanged);
            this.dgvPersonSpouce.DoubleClick += new EventHandler(dgvPersonSpouceDoubleClick);
        }
        #endregion
        ...
        protected virtual void ClassLoad(object sender, EventArgs e)
        {
            ...
            ProcStatic.SetDataGridViewColumns(this.dgvPersonSpouce, false);

        }//----------------------

        //event is raised when the class is clossing
        protected virtual void ClassClosing(object sender, FormClosingEventArgs e)
        {
        ...
        protected virtual void TextBoxKeyPressLettersOnly(object sender, KeyPressEventArgs e)
        {
            ...
            Boolean isEnable = String.IsNullOrEmpty(_personInfo.PersonSysId) ? false : true;

            this.SetPersonInformationControls(isEnable);
        }//------------------------------------
        ...
        protected virtual void txtLastNameValidated(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(_personInfo.FirstName) && !String.IsNullOrEmpty(_personInfo.LastName) && String.IsNullOrEmpty(_personInfo.PersonSysId))
            {            
                this.lnkVerify.Enabled = true;
            }
            else
        ...
        //event is raised when the mouse leave
        private void pbxGeneralInformationMouseLeave(object sender, EventArgs e)
        {
            this.pbxGeneralInformation.Image = global::BaseServices.Properties.Resources.General_Information;
        }//---------------------------

        //event is raised when the mouse is enter the control
        private void pbxGeneralInformationMouseEnter(object sender, EventArgs e)
        {
            this.pbxGeneralInformation.Image = global::BaseServices.Properties.Resources.General_Information_hover;
        }//---------------------------
        ...
        //####################################################LINKLABEL lnkCreateSpouce EVENTS###############################################
        //event is raised when the control is linked clicked
        private void lnkCreateSpouceLinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            using (PersonSpouceCreate frmCreate = new PersonSpouceCreate(_userInfo, _baseServiceManager,
                _baseServiceManager.GetPersonSpouceExcludeListStringFormat(_personInfo.PersonSpouseList, _personInfo.PersonSysId)))
            {
                frmCreate.ShowDialog(this);

                if (frmCreate.HasAdded)
                {
                    _personInfo.PersonSpouseList.Add(frmCreate.PersonSpouceInfo);

                    _baseServiceManager.InitializePersonSpouceDataGridView(this.dgvPersonSpouce, _personInfo.PersonSpouseList);
                }
            }
        }//------------------------------
        //####################################################END LINKLABEL lnkCreateSpouce EVENTS###############################################

        //####################################################DATAGRIDVIEW dgvSpouceMouseDown EVENTS####################################################
        //event is raised when the mouse is down
        private void dgvPersonSpouceMouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                ...
                switch (hitTest.Type)
                {
                    case DataGridViewHitTestType.Cell:
                        _rowIndexSpouce = hitTest.RowIndex;
                        break;
                    ...
                    default:
                        _rowIndexSpouce = -1;
                        _primaryIdSpouce = String.Empty;
                        break;
                }

                if (_rowIndexSpouce != -1)
                {
                    _primaryIdSpouce = dgvBase[_primaryIndexSpouce, _rowIndexSpouce].Value.ToString();
                }
            }
        }//-----------------------------

        //event is raised when the key is pressed
        private void dgvPersonSpouceKeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == '\r')
            {
                DataGridView dgvBase = (DataGridView)sender;
                ...
                if (dgvBase.Rows.GetRowCount(DataGridViewElementStates.Selected) > 0)
                {
                    ...
                    _primaryIdSpouce = row.Cells[_primaryIndexSpouce].Value.ToString();
                    _primaryIndexSpouce = row.Index;
                }
            }
            else
            {
        ...
        private void dgvPersonSpouceKeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void dgvPersonSpouceDataSourceChanged(object sender, EventArgs e)
        {
            DataGridView dgvBase = (DataGridView)sender;
            ...
            if (result == 1)
            {
                _primaryIdSpouce = dgvBase[_primaryIndexSpouce, 0].Value.ToString();
            }
            else
            {
                _primaryIdSpouce = String.Empty;
        ...
        private void dgvPersonSpouceSelectionChanged(object sender, EventArgs e)
        {
            DataGridView dgvBase = (DataGridView)sender;
        ...
        private void dgvPersonSpouceDoubleClick(object sender, EventArgs e)
        {
            if (_rowIndexSpouce != -1)
            {

                using (PersonSpouceUpdate frmUpdate = new PersonSpouceUpdate(_userInfo,
                    _baseServiceManager.GetDetailsPersonSpouce(_personInfo.PersonSpouseList, _primaryIdSpouce), _baseServiceManager, _personInfo.PersonSysId))
                {
                    frmUpdate.ShowDialog(this);
        ...
        //####################################################END DATAGRIDVIEW dgvSpouceMouseDown EVENTS####################################################
        #endregion
        ...
        //this procedure will remove tab spouce
        public void RemoveSpouseTab()
        {
            this.tblPersonalDetails.TabPages.Remove(this.tblSpouce);
        }//-----------------
        #endregion
