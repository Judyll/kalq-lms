...
namespace RemoteServerLib
{
    internal class Authenticate: IDisposable
    {
        ...
        public Authenticate(CommonExchange.SysAccess userInfo, Boolean isForLogIn)
        {
            if (!IsValidated(userInfo, CommonExchange.ServerCode.PrimaryServer, isForLogIn))
            {
                throw new Exception("Only authenticated users are allowed to access the system.");
            }
        }

        public Authenticate(CommonExchange.SysAccess userInfo, CommonExchange.ServerCode sCode, Boolean isForLogIn)
        {
            if (!IsValidated(userInfo, sCode, isForLogIn))
            {
                throw new Exception("Only authenticated users are allowed to access the system.");
            }
        }
        ...
        private Boolean IsValidated(CommonExchange.SysAccess userInfo, CommonExchange.ServerCode sCode, Boolean isForLogIn)
        {
            ...
            using (SqlCommand sqlComm = new SqlCommand())
            {
                ...
                sqlComm.CommandText = "lms.SelectForSystemLogInSystemUserInfo";
                ...
                sqlComm.Parameters.Add("@is_for_log_in", SqlDbType.Bit).Value = isForLogIn;
            ...
            if (hasAccess && isForLogIn)
            {
                using (SqlCommand sqlComm = new SqlCommand())
                {
                    sqlComm.Connection = sqlConn;
                    sqlComm.CommandType = CommandType.StoredProcedure;
                    sqlComm.CommandText = "lms.SelectBySystemUserIdAccessRightsGranted";
